...
const int ledPin1 = 3; // LED conectado al pin 3
const int ledPin2 = 4; // LED conectado al pin 4
...
void printResult(HUSKYLENSResult result) {
    if (result.command == COMMAND_RETURN_BLOCK) {
        ...
        Serial.print(F(", ID="));
        Serial.println(result.ID);
        //Si la dirección de la fecha indica a la mitad izquierda de la pantalla encendemos el LED1, en caso contrario el LED2
        if (result.xTarget < 160) {
          digitalWrite(ledPin1, HIGH); // Enciende el LED en el pin 3
          digitalWrite(ledPin2, LOW);  // Apaga el LED en el pin 4
        } else {
          digitalWrite(ledPin1, LOW);  // Apaga el LED en el pin 3
          digitalWrite(ledPin2, HIGH); // Enciende el LED en el pin 4
        }
    } else {
